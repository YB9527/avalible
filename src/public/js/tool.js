...
function rateSymbolSet(po, method, isadd) {
  po[method] = rateSymbol(po[method], isadd);//金额
  return po[method];

}

exports.rateSymbolSet = rateSymbolSet;

function moneySymbolPoSet(po, method, isadd) {
  po[method] = moneySymbol(po[method], isadd);//金额
  return po[method];
}

exports.moneySymbolPoSet = moneySymbolPoSet;

function recordMoneySymbol(collectionrecord,  isadd) {
  ...
  for (let i in collectionrecord.percentagepayvalueArray) {
    ...
    collectionrecord.percentagepayvalueArray[i] = moneySymbol(collectionrecord.percentagepayvalueArray[i], isadd);
  }
}
exports.recordMoneySymbol = recordMoneySymbol;
...
function projectMoneySymbol(project, isadd) {
  ...
  for (let i in project.percentages) {
    ...
    project.percentagepaysteponeratevaluearray[i] = moneySymbol(project.percentagepaysteponeratevaluearray[i], isadd);
    project.percentagepaysteptworatevaluearray[i] = moneySymbol(project.percentagepaysteptworatevaluearray[i], isadd);
    project.percentagepaystepthreeratevaluearray[i] = moneySymbol(project.percentagepaystepthreeratevaluearray[i], isadd);
  }
