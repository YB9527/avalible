<template>
  <div>
    <div v-show="showreportpage">
      ...
      <!--合并表格-->
      <div style="margin: 0px 0px 50px 10%" >



        <table :id="'table'+0" class="table table-bordered table-hover" v-if="historyreports.total">
          <thead>
          <tr>
            <th>
              <div class="btn-group " style="width: 130px;height: 33px;" v-if="historyreports.total" >
                ...
                <div class="dropdown-menu">
                  <button type="button" class="btn btn-outline-dark dropdown-item"
                          @click="exportExcel('table0','合计')">
                    Excel
                  </button>
                  <button type="button" class="btn btn-outline-dark dropdown-item"
                          @click="exportPDF('table0','合计')">
                    PDF
                  </button>
                </div>
              </div>
            </th>
            <th colspan="3" style="text-align: center"><h4>合计&nbsp;&nbsp;{{historyreports.length}}&nbsp;&nbsp;个项目</h4></th>
            <!-- <th style="border-top: 0px red solid"></th>-->
          </tr>
          <tr>
            <th style="width: 150px">项目合同金额:</th>
            <th style="width: 150px;text-align: right">{{historyreports.total.contrafacevalue|moneyformat}}</th>
            <th style="width: 150px">有效比例金额：</th>
            <th style="width: 150px;text-align: right">{{historyreports.total.validratefee|moneyformat }}</th>
          </tr>
          <tr>
            <th style="width: 150px">总提成:</th>
            <th style="width: 150px;text-align: right">{{historyreports.total.percentagetotal|moneyformat }}</th>
            <th style="width: 150px">总分红：</th>
            <th style="width: 150px;text-align: right">{{historyreports.total.fundpay|moneyformat }}</th>
          ...
          <tr>
            ...
            <th style="text-align: right">
              {{historyreports.total.amountcollectedtotallast |moneyformat}}
            </th>
            <th>本次收款：</th>
            <th style="text-align: right">{{historyreports.total.amountcollectedtotal |moneyformat}}
            </th>

          </tr>
          <tr>
            <th colspan="4">提成、分红 详细</th>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="text-align: right">
              {{historyreports.total.tichenglastqianding |moneyformat}}
            </td>
            <td style="text-align: right">
              {{historyreports.total.tichengqianding |moneyformat}}
            </td>
          </tr>
          <tr>
            ...
            <td>收款绑定</td>
            <td style="text-align: right">
              {{historyreports.total.tichenglastshoukuan |moneyformat}}

            </td>
            <td style="text-align: right">
              {{historyreports.total.tichengshoukuan |moneyformat}}
            </td>
          </tr>
          <tr>
            ...
            <td>年终</td>
            <td style="text-align: right">
              {{historyreports.total.tichenglastnianzhong |moneyformat}}
            </td>
            <td style="text-align: right">
              {{historyreports.total.tichengnianzhong |moneyformat}}
            </td>
          </tr>
          <tr>
            <td style="text-align: right">
            ...
            <th style="text-align: right"> {{historyreports.total.tichengzhiqain |moneyformat}}</th>
            <th style="text-align: right"> {{historyreports.total.tichengxianzai |moneyformat}}</th>
          ...
          <tr>
            ...
            <td>合同签订</td>
            <td style="text-align: right">
              {{historyreports.total. fenhonglastqianding |moneyformat}}
            </td>
            <td style="text-align: right">
              {{historyreports.total.fenhongqianding |moneyformat}}
            </td>
          </tr>
          <tr>
            ...
            <td>收款绑定</td>
            <td style="text-align: right">
              {{historyreports.total.fenhonglastshoukuan |moneyformat}}
            </td>
            <td style="text-align: right">
              {{historyreports.total.fenhongshoukuan |moneyformat}}
            </td>
          ...
          <tr>
            <td>年终</td>
            <td style="text-align: right">
              {{historyreports.total.fenhonglastnianzhong |moneyformat}}
            </td>
            <td style="text-align: right">
              {{historyreports.total.fenhongnianzhong |moneyformat}}
            </td>
          </tr>
          <tr>
            <td style="text-align: right">
            ...
            <th style="text-align: right"> {{historyreports.total.fenghongzhiqian |moneyformat}}</th>
            <th style="text-align: right"> {{historyreports.total.fenghongxianzai |moneyformat}}</th>
          </tr>
          <tr>
            <th colspan="1">
              合计：
            ...
            <th style="text-align: right">{{historyreports.total.zhiqianheji |moneyformat}}
            </th>
            <th style="text-align: right">{{historyreports.total. xianzaiheji |moneyformat}}
            </th>
          </tr>
          </tbody>
        </table>
      ...
      <div style="text-align: center;margin: 0px 0px 50px 10%" v-for="(current,i) in historyreports">
        ...
        <table :id="'table'+(1+i)" class="table table-bordered table-hover">
          ...
          <tr>
           <td>
             <div>
               <div class="btn-group " style="width: 130px;height: 33px;float: left;margin-right: 20px;margin-bottom: 5px" v-if="showreportpagetop" >
                 <button type="button" class="btn btn-outline-dark dropdown-toggle"
                         data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                   下载
                 </button>
                 <div class="dropdown-menu">
                   <button type="button" class="btn btn-outline-dark dropdown-item"
                           @click="exportExcel('table'+(i+1),current.monthreport.project.projectname)">
                     Excel
                   </button>
                   <!-- <button type="button" class="btn btn-outline-dark dropdown-item"
                            @click="exportPDF('table'+(i+1),current.monthreport.project.projectname)">
                      PDF
                    </button>-->
                 </div>
               </div>
               ...
               <button type="button" @click="backProject(current.monthreport.project._id)" v-if="showreportpagetop"
                       class="form-control  btn-outline-dark form-control-sm" style="width: 130px">
                 当前项目
               </button>
             </div>
           </td>
            <th :colspan="3+current.colspan"><h4>{{current.monthreport.project.projectname}}</h4></th>
            <!-- <th style="border-top: 0px red solid"></th>-->
          </tr>
          <tr>
            <th style="width: 150px">项目合同金额:</th>
    ...
    <CreateProject @editpofunction_back="editpofunction_back" :editpo="editpo" v-if="showcreateproject"></CreateProject>
    ...
    methods: {
      exportPDF(id, filename) {
        ...
        html2Canvas(element, {
          ...
          while (renderedHeight < canvas.height) {
            ...
            console.log(ctx.getImageData(0, renderedHeight, canvas.width, Math.min(imgHeight, canvas.height - renderedHeight)), 0, 0);
            renderedHeight += imgHeight;
            if (renderedHeight < canvas.height)
              pdf.addPage();//如果后面还有内容，添加一个空页
            //delete page;
          }
          pdf.save( filename  + self.lookshortreportdate + '.pdf');
          console.log(1)
        });
      ...
      setdata(historyreports) {
        ...
        for (let historyreport of historyreports) {
          ...
          if (lastreport) {
            tool.projectMoneySymbol(lastreport.project, false);
